...
It can be easy to think of 'tasks' as being a bit like the datasets used in SFT, but since these are inputs or problems for which we don't have golden ground truth answers, it's still quite different. The approach with RL is that the GRPO algorithm (during training) will attempt to solve those problems multiple times, and then it will use the reward to differentiate between those attempts and then based on which attempts were relatively stronger it will update weights of the model/policy accordingly.
...
- Trainer: Not much to say about this except to say that this is usually done by different frameworks than what one is used to with standard machine learning or fine-tuning. Part of the difference is that you are doing actual inference during the training (multiple times over each problem so you can compare between them) so you are both serving the model and updating weights at the same time. (I *think* this basically means you have the model loaded into RAM two times, or something like that? I'm sure they have made efficiency improvements for this, but that's the mental model I have at the moment.)
...
From what I read, it seems in 2025-2026 there was a bit of a shift away from highly ornate reward functions (made up of carefully weighted multi-part sets of assesments, and also known as 'dense rewards') towards simpler outcome-driven rewards (aka 'sparse rewards'). There's a lot of nuance here and I'm interested to learn why this happened and where it still makes sense to have domain experts in the loop creating these reward functions. It seems to me even if models are getting stronger and even if the algorithm is strong enough to update the weights based on much vaguer outcome rewards, there needs to be *some* more to it beyond just that...
